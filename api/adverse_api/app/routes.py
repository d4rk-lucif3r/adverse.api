        ...
        if mode == "full":
            ...
            for document in cursor:
                ...
                try:
                    ...
                    document["City_State_mentioned_under_the_news"] = document.pop(
                        "City/ State mentioned under the news"
                    )
                    document["HDFC_Bank_Name_under_News_Article"] = document.pop(
                        "HDFC Bank Name under News / Article"
                    )
                    document["Key_word_Used_foruuidentify_the_article"] = document.pop(
                        "Key word Used for identify the article"
                    )
                    document["Person_Name_mentioned_in_the_news"] = document.pop(
                        "Person Name mentioned in the news"
                    )
                    document["Organization_Name_mentioned_in_the_news"] = document.pop(
                        "Organization Name mentioned in the news"
                    )
                    document["Source_Name"] = document.pop("Source Name")

                    if document["Source_Name"] in source2name.keys():
                        document["Source_Name"] = source2name[document["Source_Name"]]
                    ...
                    if document["Article_Date"]:
                        document["Article_Date"] = parse(
                            document["Article_Date"].split("+")[0]
                        )
                        document["Article_Date"] = document["Article_Date"].strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
            ...
            search_results = list(
                {v["Web_link_of_news"]: v for v in search_results}.values()
            )

            return jsonify(
                {
                    "news_source_id": ids["news_source_ids"],
            ...
            if _request["date"]:
                ...
                collection_batches = db["adverse_db"]
                cursor = collection_batches.find(
                    {"batch_id": {"$in": _batch_ids}}, {"_id": False}
                )
                # cursor = collection_batches.find({})
                ...
                for document in cursor:
                    ...
                    try:
                        ...
                        document["Article_Date"] = document.pop("Article Date")
                        document["City_of_News_Paper"] = document.pop(
                            "City of News Paper"
                        )
                        document["City_State_mentioned_under_the_news"] = document.pop(
                            "City/ State mentioned under the news"
                        ...
                        document[
                            "Key_word_Used_foruuidentify_the_article"
                        ] = document.pop("Key word Used for identify the article")
                        document["Person_Name_mentioned_in_the_news"] = document.pop(
                            "Person Name mentioned in the news"
                        )
                        document[
                            "Organization_Name_mentioned_in_the_news"
                        ] = document.pop("Organization Name mentioned in the news")
                        document["Source_Name"] = document.pop("Source Name")

                        if document["Source_Name"] in source2name.keys():
                            document["Source_Name"] = source2name[
                                document["Source_Name"]
                            ]

                        document["Source_of_Info"] = document.pop("Source of Info")
                        document["Web_link_of_news"] = document.pop("Web link of news")
                        document["created_date"] = document["created_date"].split(".")[
                            0
                        ]
                        ...
                        if document["Article_Date"]:
                            ...
                            document["Article_Date"] = document[
                                "Article_Date"
                        ...
                        if document["City_of_News_Paper"] == "National":
                            document["City_of_News_Paper"] = CityOfNewspaper(
                                document["Web_link_of_news"]
                            )
            ...
            if (
                ("keywords" in _keys)
                and ("news_source_ids" in _keys)
                and ("exclude" in _keys)
                and ("fp_name" in _keys)
                and ("fp_city" in _keys)
                and ("cities" in _keys)
                and ("names" in _keys)
                and ("excludeorg" in _keys)
            ):
                # if _request['keywords'] and _request['news_source_ids'] and _request['fp_name'] and _request["fp_city"]:
                print(
                    "this is request for name, city, keywords, news_source_id and false positives and exclude, excludeorg"
                )
                _request["fp_name"] = _request["fp_name"].split(
                    ","
                )  # + ['AGRA', 'Union', 'Budget', 'Centre', 'Getty Images', 'AFP/Getty']
                ...
                _request["names"] = _request["names"].split(
                    ","
                )  # + ['AGRA', 'Union', 'Budget', 'Centre', 'Getty Images', 'AFP/Getty']
                ...
                update_ids_dbs(
                    ...
                    news_source_ids=_request["news_source_ids"],
                    exclude=_request["exclude"],
                    fp_name=_request["fp_name"],
                    fp_city=_request["fp_city"],
                ...
                # if _request['keywords'] and _request['news_source_ids'] and _request['fp_name'] and _request["fp_city"]:
                print(
                    "this is request for exclude, city, keywords, fp and news_source_id"
                )
                _request["fp_name"] = _request["fp_name"].split(
                    ","
                )  # + ['AGRA', 'Union', 'Budget', 'Centre', 'Getty Images', 'AFP/Getty']
                ...
                _request["cities"] = _request["cities"].split(
                    ","
                )  # + ['AGRA', 'Union', 'Budget', 'Centre', 'Getty Images', 'AFP/Getty']
                _request["cities"] = ",".join(list(set(_request["cities"])))
                ...
                update_ids_dbs(
                    ...
                    news_source_ids=_request["news_source_ids"],
                    excludeorg=_request["excludeorg"],
                )  # , _request['fp_name'], _request["fp_city"])
                # keywords = _request['keywords'].split(',')
                # news_source_id = _request['news_source_ids'].split(',')
                    ...
                    for document in cursor:
                        ...
                        if document["uuid"] in uuids:
                            ...
                            try:
                                ...
                                article = Article(
                                    document["Web link of news"], config=config
                                )
                                article.download()
                                article.parse()
                                ...
                                if lang == "mr":
                                    ...
                                    if lang == "mr":
                                        translate_text2 = translator2.translate(
                                            text, lang_tgt="en"
                                        )
                                ...
                                person = [
                                    i
                                    for i in person
                                    if not any([i in a for a in person if a != i])
                                ]
                                ...
                                city = [
                                    i
                                    for i in city
                                    if not any([i in a for a in city if a != i])
                                ]
                                ...
                                if document["City of News Paper"] == "National":
                                    document["City of News Paper"] = CityOfNewspaper(
                                        document["Web link of news"]
                                    )

                                if document["Source Name"] in source2name.keys():
                                    document["Source Name"] = source2name[
                                ...
                                return jsonify(
                                    {
                                        ...
                                        "date_of_response": None,
                                        "mode_of_search": mode,
                                        "search_results": "successfully updated:%s"
                                        % document["uuid"],
                                    }
                                )
                                ...
                                return jsonify(
                                    {
                                        ...
                                        "date_of_response": None,
                                        "mode_of_search": mode,
                                        "search_results": "parse_existing exception:%s"
                                        % str(e),
                                    }
                                )
            ...
            if _request["keywords"] and _request["urltobesearched"]:
                ...
                if keywords and urltobesearched:
                    profile = {
                        "Person_Name_mentioned_in_the_news": "",
                        "Organization_Name_mentioned_in_the_news": "",
                    ...
                    try:
                        ...
                        if KORG:
                            ...
                            profile["Key_word_Used_foruuidentify_the_article"] += (
                                ", ".join(KORG) + ", "
                            )
                            ...
                            for keyword in keywords:
                                ...
                                if keyword.isupper():
                                    ...
                                    if len(keyword.split(" ")) > 1:
                                        ...
                                        _keyword = [
                                            x.strip() for x in _keyword if x.strip()
                                        ]
                        ...
                        profile["created_date"] = profile["created_date"].strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        profile["Organization_Name_mentioned_in_the_news"] = profile[
                            "Organization_Name_mentioned_in_the_news"
                        ].split(",")
                        # print(profile['Organization_Name_mentioned_in_the_news'])
                        profile["Organization_Name_mentioned_in_the_news"] = [
                            x.strip()
                            for x in profile["Organization_Name_mentioned_in_the_news"]
                            if x.strip()
                        ]
                        profile["Organization_Name_mentioned_in_the_news"] = list(
                            set(profile["Organization_Name_mentioned_in_the_news"])
                        )
                        profile["Organization_Name_mentioned_in_the_news"] = [
                            x.strip()
                            for x in profile["Organization_Name_mentioned_in_the_news"]
                        ...
                        profile["Person_Name_mentioned_in_the_news"] = (
                            profile["Person_Name_mentioned_in_the_news"].split(",")
                            + profile["Organization_Name_mentioned_in_the_news"]
                        )
                        # print(profile['Person Name mentioned in the news'])
                        profile["Person_Name_mentioned_in_the_news"] = [
                            x.strip()
                            for x in profile["Person_Name_mentioned_in_the_news"]
                            if x.strip()
                        ]
                        profile["Person_Name_mentioned_in_the_news"] = list(
                            set(profile["Person_Name_mentioned_in_the_news"])
                        )
                        profile["Person_Name_mentioned_in_the_news"] = [
                            i
                            for i in profile["Person_Name_mentioned_in_the_news"]
                            if not any(
                                [
                                    i in a
                                    for a in profile[
                        ...
                        person_dict = {
                            k.lower(): k
                            for k in profile["Person_Name_mentioned_in_the_news"]
                        }
                        profile["Person_Name_mentioned_in_the_news"] = list(
                            person_dict.values()
                        )
                        ...
                        for name in profile["Person_Name_mentioned_in_the_news"]:
                            if name.lower() in cities["cities"]:
                                profile["Person_Name_mentioned_in_the_news"].remove(
                                    name
                                )
                                profile["City_State_mentioned_under_the_news"] += (
                                    name + ", "
                        ...
                        profile["Person_Name_mentioned_in_the_news"] = " | ".join(
                            profile["Person_Name_mentioned_in_the_news"]
                        )
                        profile["Organization_Name_mentioned_in_the_news"] = " | ".join(
                            profile["Organization_Name_mentioned_in_the_news"]
                        ...
                        profile["City_State_mentioned_under_the_news"] = [
                            x.strip()
                            for x in profile["City_State_mentioned_under_the_news"]
                            if x.strip()
                        ]
                        profile["City_State_mentioned_under_the_news"] = list(
                            set(profile["City_State_mentioned_under_the_news"])
                        )
                        profile["City_State_mentioned_under_the_news"] = [
                            i
                            for i in profile["City_State_mentioned_under_the_news"]
                            if not any(
                                [
                                    i in a
                                    for a in profile[
                        ...
                        city_dict = {
                            k.lower(): k
                            for k in profile["City_State_mentioned_under_the_news"]
                        }
                        profile["City_State_mentioned_under_the_news"] = list(
                            city_dict.values()
                        )
                        ...
                        profile["City_State_mentioned_under_the_news"] = " | ".join(
                            profile["City_State_mentioned_under_the_news"]
                        )
                        profile["Source_of_Info"] = "Newspaper"
                        profile["Key_word_Used_foruuidentify_the_article"] = fnc_(
                            profile["Key_word_Used_foruuidentify_the_article"]
                        )
                        profile["uuid"] = f1.uuid4()
                        # profile.pop('Organization_Name_mentioned_in_the_news')

                        profile["City_of_News_Paper"] = CityOfNewspaper(
                            profile["Web_link_of_news"]
                        )
                        ...
                        print('[DEBUG] City :', profile["City_State_mentioned_under_the_news"])
                        return jsonify(
                            {
                                "news_source_ids": ids["news_source_ids"],
                                "last_updated_time": dbs[-1]["RunDate"],
                    ...
                    return jsonify(
                        {
                            ...
                            "mode_of_search": mode,
                            "search_results": [
                                "Please correct keywords and urltobesearched"
                            ],
                        }
                    )
        ...
        if mode == "full":
            ...
            if _request["date"]:
                ...
                for result in search_results:
                    if (
                        datetime.strptime(result["Article Date"], "%Y-%m-%d %H:%M:%S")
                        > date
                    ):
                        _search_results.append(result)

                last_updated_time = datetime.strptime(
                    "2021-02-27 21:02:45", "%Y-%m-%d %H:%M:%S"
                )
            ...
            if _request["keywords"] and _request["news_source_ids"]:
                ...
                for source_id in news_source_ids:
                    if source_id not in [
                        ...
                        "5966436a-2f26-4305-95a3-fcda913d621d",
                        "ad60ab7b-906b-467d-b29e-92f200eb88fe",
                        "52d0de86-1525-417d-b8fd-2158f1256c38",
                        "557e51e6-b04c-4be1-a4fa-aff4b7e1c37b",
                        "4dfab6d8-8246-469b-9e19-7ddbb55d806d",
                        ...
                        return jsonify(
                            {
                                ...
                                "mode_of_search": mode,
                                "search_results": [
                                    "Please provide news_source_id only from 28 newspaper"
                                ],
                            }
                        )
                ...
                return jsonify(
                    {
                        ...
                        "mode_of_search": mode,
                        "search_results": [
                            "Please provide atleast one news_source_ids"
                        ],
                    }
                )
            else:
