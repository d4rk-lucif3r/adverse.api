...
def update_current_names(names=""):
    ...
    post = collection_batches.find_one(
        {"_id": bson.objectid.ObjectId("608fd0093532c6e24763040e")}
    )
    if post:
        if names:
...
def update_fp(fp_name="", fp_city="", excludeorg="", exclude=""):
    ...
    collection_batches = db["fp_list"]
    post = collection_batches.find_one(
        {"_id": bson.objectid.ObjectId("60799419ba4eda7932fc2ec2")}
    )
    if post:
...
def soup_text(soup, sourcename):
    ...
    dictionary = {
        ...
        "indianexpress.com": {
            "Headlines": {"h1": ["native_story_title"]},
            "Synopsis": {"h2": ["synopsis"]},
            "Text": {"div": ["amp-wp-content", "full-details", "pcl-full-content"]},
        },
        "www.bbc.com": {
            "Headlines": {
                "h1": [
...
def search(pep_name):
    ...
    request_body = (
        '{ "query": { "multi_match" : { "query":    "(%s)", "fields": [ "Person Name mentioned in the news", "Organization Name mentioned in the news", "Key word Used for identify the article", "Web link of news", "Source Name" ], "operator":   "and", "fuzziness" : "AUTO" } } }'
        % pep_name
    )
    ...
    response = elasticsearch_curl(uri, verb="get", json_body=request_body)
...
def search_pep(pep_name):
    ...
    request_body = (
        '{ "query": { "multi_match" : { "query":    "(%s)", "fields": [ "full_name", "aliases", "first_name", "last_name", "native_name", "birth_name", "father", "mother", "siblings", "spouse", "children", "relatives" ], "operator":   "and" } } }'
        % pep_name
    )
...
def search_spouse(pep_name):
    ...
    request_body = (
        '{ "query": { "multi_match" : { "query":    "(%s)", "fields": [ "father", "mother", "siblings", "children", "relatives" ], "operator":   "and" } } }'
        % pep_name
    )
...
def search_spouse2(pep_name):
    ...
    request_body = (
        '{ "query": { "multi_match" : { "query":    "(%s)", "fields": [ "spouse" ], "operator":   "and" } } }'
        % pep_name
    )
...
def search_relatives(pep_name):
    ...
    request_body = (
        '{ "query": { "multi_match" : { "query":    "(%s)", "fields": [ "father", "mother", "siblings", "spouse", "children" ], "operator":   "and" } } }'
        % pep_name
    )
...
def qcode2val(response):
    ...
    if response["political_associations"]:
        response["political_associations"] = ast.literal_eval(
            response["political_associations"]
        )
        response["political_associations"] = [
            qregex.sub(None, qcode) for qcode in response["political_associations"]
        ]
    if response["relatives"]:
        response["relatives"] = ast.literal_eval(response["relatives"])
        response["relatives"] = [
            qregex.sub(None, qcode) for qcode in response["relatives"]
        ]
    if response["occupation"]:
        response["occupation"] = ast.literal_eval(response["occupation"])
        response["occupation"] = [
            qregex.sub(None, qcode) for qcode in response["occupation"]
        ]
    if response["spouse"]:
    ...
    if response["residence"]:
        ...
        response["residence"] = [
            qregex.sub(None, qcode) for qcode in response["residence"]
        ]
    if response["children"]:
        response["children"] = ast.literal_eval(response["children"])
        response["children"] = [
            qregex.sub(None, qcode) for qcode in response["children"]
        ]
    if response["positions_held"]:
        response["positions_held"] = ast.literal_eval(response["positions_held"])
        response["positions_held"] = [
            qregex.sub(None, qcode) for qcode in response["positions_held"]
        ]
    if response["mother"]:
