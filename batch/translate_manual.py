...
def translate(text):
    translation = language_translator.translate(
        text=text, model_id="{}-en".format(auto_detect_language(text))
    ).get_result()
...
def parse_soup(nation, url, soup):
    ...
    write_obj = open(
        "./result_database/manual_data.csv", "a+", newline="", encoding="utf-8"
    )
    ...
    for items in content:
        ...
        (
            ...
            twitter_url,
            other_urls,
        ) = sc.link_parser(items[1])
        l1.extend(
            (url, image_url, facebook_url, instagram_url, twitter_url, other_urls)
        )  # urls
...
def postprocess():
    ...
    try:
        df = pd.read_csv(
            "result_database/manual_data.csv",
            names=[
                "Prefix",
                "Name",
                "Designation",
