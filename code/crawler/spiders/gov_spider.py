...
class govSpider(scrapy.Spider):
    ...
    redirected = []
    def __init__(self, filename= 'starter_sites.csv', *args, **kwargs):
        ...
        for i in range(len(nations)):
            ...
            for item in self.english:
                if item in site.lower().split('/'):
                    self.nations_dict[self.ret_domain(site)] = (nations.iloc[i,1].strip(),'en')
                    break
            else:
                self.nations_dict[self.ret_domain(site)] = (nations.iloc[i,1].strip(),None)
