...
pd.DataFrame(data=cities, index=range(len(cities)), columns=["cities"]).to_csv(
    "Cities.csv", index=None
)
...
pd.DataFrame(data=names, index=range(len(names)), columns=["names"]).to_csv(
    "names.csv", index=None
)
...
pd.DataFrame(data=fp_name, index=range(len(fp_name)), columns=["fp_name"]).to_csv(
    "fp_name.csv", index=None
)

pd.DataFrame(data=fp_city, index=range(len(fp_city)), columns=["fp_city"]).to_csv(
    "fp_city.csv", index=None
)
